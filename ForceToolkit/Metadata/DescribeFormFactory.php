...
use
    ...
    CiscoSystems\SalesForceBundle\ForceToolkit\Soap\Mapping\Type\layoutComponentType,
    CiscoSystems\SalesForceBundle\ForceToolkit\Soap\Mapping\Type\fieldType,
    CiscoSystems\SalesForceBundle\Common\Cache\GenericCacheInterface,
    CiscoSystems\SalesForceBundle\ForceToolkit\Metadata\Cache\MetadataCacheInterface
;
...
class DescribeFormFactory implements DescribeFormFactoryInterface
{
     ...
     * @abstract
     * @param $sobjectType
     * @param string|\CiscoSystems\SalesForceBundle\ForceToolkit\Soap\Mapping\Type\ID|null $recordTypeId
    ...
    public function getDescribe($sobjectType, $recordTypeId = null)
    {
        ...
        /** @var $recordTypeMapping \CiscoSystems\SalesForceBundle\ForceToolkit\Soap\Mapping\RecordTypeMapping */
        $recordTypeMapping = count($recordTypeMappings) > 0 ? $recordTypeMappings[0] : null;

        // FALLBACK 2: DEFAULT RT MAPPING
        /** @var $rtm \CiscoSystems\SalesForceBundle\ForceToolkit\Soap\Mapping\RecordTypeMapping */
        ...
        /** @var $layout \CiscoSystems\SalesForceBundle\ForceToolkit\Soap\Mapping\DescribeLayout */
        foreach($describeLayoutResult->getLayouts() AS $layout)
        {
            // FOUND LAYOUT!
            if((string)$layout->getId() === (string)$recordTypeMapping->getLayoutId())
            {
                /** @var $editLayoutSection \CiscoSystems\SalesForceBundle\ForceToolkit\Soap\Mapping\DescribeLayoutSection */
                foreach($layout->getEditLayoutSections() AS $editLayoutSection)
                {
                    /** @var $layoutRow \CiscoSystems\SalesForceBundle\ForceToolkit\Soap\Mapping\DescribeLayoutRow */
                    foreach($editLayoutSection->getLayoutRows() AS $layoutRow)
                    {
                        /** @var $layoutItem \CiscoSystems\SalesForceBundle\ForceToolkit\Soap\Mapping\DescribeLayoutItem */
                        foreach($layoutRow->getLayoutItems() AS $layoutItem)
                        {
                            if($layoutItem->getLayoutComponents())
                            {
                                /** @var $component \CiscoSystems\SalesForceBundle\ForceToolkit\Soap\Mapping\DescribeLayoutComponent */
        ...
        /* @var $picklistForRecordType \CiscoSystems\SalesForceBundle\ForceToolkit\Soap\Mapping\PicklistForRecordType */
        foreach($recordTypeMapping->getPicklistsForRecordType() AS $picklistForRecordType)
        {
            /** @var $originalField \CiscoSystems\SalesForceBundle\ForceToolkit\Soap\Mapping\Field */
            $originalField = $fieldsByName[$picklistForRecordType->getPicklistName()];

            $originalPicklistEntries = array();

            /** @var $originalPicklistEntry \CiscoSystems\SalesForceBundle\ForceToolkit\Soap\Mapping\PicklistEntry */
            ...
            /** @var $picklistEntry \CiscoSystems\SalesForceBundle\ForceToolkit\Soap\Mapping\PicklistEntry */
